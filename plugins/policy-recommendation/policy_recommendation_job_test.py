...
def test_remove_meaningless_labels(test_input, expected_labels):
    pod_labels = remove_meaningless_labels(test_input)
    assert pod_labels == expected_labels

@pytest.mark.parametrize("test_input, expected_proto_string", [
    (
        6, 
        "TCP"
    ),
...
def test_get_protocol_string(test_input, expected_proto_string):
    proto_string = get_protocol_string(test_input)
...
@pytest.mark.parametrize("test_input, expected_sql_query", [
    ...
    (
        ...
        "SELECT {} FROM {} WHERE ingressNetworkPolicyName == '' AND egressNetworkPolicyName == '' AND flowStartSeconds >= '2022-01-01 00:00:00' GROUP BY {} LIMIT 100".format(", ".join(FLOW_TABLE_COLUMNS), table_name, ", ".join(FLOW_TABLE_COLUMNS))
    ),
    (
    ...
    (
        ...
        "SELECT {} FROM {} WHERE ingressNetworkPolicyName == '' AND egressNetworkPolicyName == '' AND flowStartSeconds >= '2022-01-01 00:00:00' AND flowEndSeconds < '2022-01-01 23:59:59' GROUP BY {} LIMIT 100".format(", ".join(FLOW_TABLE_COLUMNS), table_name, ", ".join(FLOW_TABLE_COLUMNS))
    ),
])
...
@pytest.mark.parametrize("test_input, expected_egress", [
    (
        (flow_rows[0], False),
        ('antrea-test#{"podname":"perftest-a"}', ('', 'antrea-test#{"podname":"perftest-b"}#5201#TCP'))  
    ),
    (
        (flow_rows[1], False),
        ('antrea-test#{"podname":"perftest-a"}', ('', 'antrea-e2e#perftestsvc'))  
    ),
    (
        (flow_rows[2], False),
        ('antrea-test#{"podname":"perftest-a"}', ('', '192.168.0.1#80#TCP'))  
    ),
    (
        (flow_rows[0], True),
        ('antrea-test#{"podname":"perftest-a"}', ('', 'antrea-test#{"podname":"perftest-b"}#5201#TCP'))  
    ...
    (
        (flow_rows[1], True),
        ('antrea-test#{"podname":"perftest-a"}', ('', 'antrea-test#{"podname":"perftest-c"}#5201#TCP'))
    ),
    (
        (flow_rows[2], True),
        ('antrea-test#{"podname":"perftest-a"}', ('', '192.168.0.1#80#TCP'))  
...
def test_map_flow_to_egress(test_input, expected_egress):
    flow, k8s = test_input
    egress = map_flow_to_egress(flow, k8s)
    assert egress == expected_egress

@pytest.mark.parametrize("test_input, expected_egress", [
    (
        flow_rows[1],
        ('antrea-test#{"podname":"perftest-a"}', 'antrea-e2e/perftestsvc:5201#5201#TCP')
    )
])
def test_map_flow_to_egress_svc(test_input, expected_egress):
    egress = map_flow_to_egress_svc(test_input)
    assert egress == expected_egress

@pytest.mark.parametrize("test_input, expected_ingress", [
    (
        flow_rows[0],
        ('antrea-test#{"podname":"perftest-b"}', ('antrea-test#{"podname":"perftest-a"}#5201#TCP', ""))
    ),
    (
        flow_rows[1],
        ('antrea-test#{"podname":"perftest-c"}', ('antrea-test#{"podname":"perftest-a"}#5201#TCP', ""))
    ),
    (
        flow_rows[2],
...
def test_map_flow_to_ingress(test_input, expected_ingress):
    ingress = map_flow_to_ingress(test_input)
    assert ingress == expected_ingress

@pytest.mark.parametrize("test_input, expected_peer", [
    (
        (
            ("", 'antrea-test#{"podname":"perftest-a"}#5201#TCP'),
            ('antrea-test#{"podname":"perftest-a"}#5201#TCP', ""),
        ),
        ('antrea-test#{"podname":"perftest-a"}#5201#TCP', 'antrea-test#{"podname":"perftest-a"}#5201#TCP')
    ),
    (
        (
            ('antrea-test#{"podname":"perftest-a"}#5201#TCP', ""),
    ...
    (
        ...
        ('antrea-test#{"podname":"perftest-a"}#5201#TCP', 'antrea-test#{"podname":"perftest-b"}#5201#TCP')
    ),
])
def test_combine_network_peers(test_input, expected_peer):
    peer1, peer2 = test_input
    network_peer = combine_network_peers(peer1, peer2)
    assert network_peer == expected_peer

@pytest.mark.parametrize("test_input, expected_egress_rule", [
    (
        'wrongformat',
        "",
    ),
...
def test_generate_k8s_egress_rule(test_input, expected_egress_rule):
    if test_input == "wrongformat":
        with pytest.raises(SystemExit) as pytest_wrapped_e:
                generate_k8s_egress_rule(test_input)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 1
    else:
        egress_rule = generate_k8s_egress_rule(test_input)
        assert egress_rule == expected_egress_rule

@pytest.mark.parametrize("test_input, expected_ingress_rule", [
    (
        'wrongformat',
        "",
    ),
...
def test_generate_k8s_ingress_rule(test_input, expected_ingress_rule):
    if test_input == "wrongformat":
        with pytest.raises(SystemExit) as pytest_wrapped_e:
                generate_k8s_ingress_rule(test_input)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 1
    else:
        ingress_rule = generate_k8s_ingress_rule(test_input)
        assert ingress_rule == expected_ingress_rule

@pytest.mark.parametrize("test_input, expected_policy_name", [
    (
        "recommend-k8s-np",
        "recommend-k8s-np-y0cq6"
    ),
    (
        "recommend-allow-anp",
        "recommend-allow-anp-y0cq6"
    )
])
def test_generate_policy_name(test_input, expected_policy_name):
    random.seed(0)
    policy_name = generate_policy_name(test_input)
    assert policy_name == expected_policy_name

@pytest.mark.parametrize("test_input, expected_cg_name", [
    (
        ("antrea-e2e", "perfsvc"),
        "cg-antrea-e2e-perfsvc"
    )
])
def test_get_svc_cg_name(test_input, expected_cg_name):
    ns, svc_name = test_input
    cg_name = get_svc_cg_name(ns, svc_name)
...
def test_generate_anp_egress_rule(test_input, expected_egress_rule):
    if test_input == "wrongformat":
        with pytest.raises(SystemExit) as pytest_wrapped_e:
                generate_anp_egress_rule(test_input)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 1
    else:
        egress_rule = generate_anp_egress_rule(test_input)
        assert egress_rule == expected_egress_rule
...
def test_generate_anp_ingress_rule(test_input, expected_ingress_rule):
    if test_input == "wrongformat":
        with pytest.raises(SystemExit) as pytest_wrapped_e:
                generate_anp_ingress_rule(test_input)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 1
    else:
        ingress_rule = generate_anp_ingress_rule(test_input)
        assert ingress_rule == expected_ingress_rule
                ...
                "perftest-a": {
                    ...
                    "spec": {
                        ...
                        "egress": [
                            {
                                "action": "Allow",
                                "toServices": [{"name": "perftestsvc", "namespace": "antrea-e2e"}],
                            },
                            {
                                "action": "Allow",
                                "ports": [{"port": 5201, "protocol": "TCP"}],
